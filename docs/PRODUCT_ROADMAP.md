...
> 目标：把 `EasyAgent` 从当前的可运行原型，逐步推进到三个可落地的最终形态：
...
目标：让 `EasyAgent` 真正具备长期承接三种产品形态的基础。
...
- 优先做独立 `bridge` 进程
- 通过 `easyagent serve` 的 API / WebSocket 与内核通信
- 飞书侧优先使用长连接事件模式
...
`EasyAgent` 后续最重要的不是继续分散做入口，而是先把共用内核做厚，再以 `CLI` 为主线，`Server + Feishu` 为服务化方向，`Desktop` 为产品化展示和轻量工作台方向逐步推进。
